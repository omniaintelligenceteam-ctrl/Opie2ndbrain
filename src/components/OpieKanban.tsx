...
export default function OpieKanban() {
  ...
  const activities = [{ id: '1', action: 'Added security monitoring', time: 'Just now' }, { id: '2', action: 'Dashboard updated', time: '5 min ago' }]
  const helpers = [{ id: '1', name: 'Researcher', status: 'idle' }, { id: '2', name: 'Coder', status: 'idle' }]
  const memories = [{ id: '1', text: 'Wes likes brutal honesty', category: 'preference', importance: 'critical' }, { id: '2', text: 'Omnia Light Scape Pro - AI SaaS', category: 'project', importance: 'high' }]
  const security = { lastCheck: 'Just now', status: 'secure', checks: [{ name: 'API Keys', status: 'ok', detail: 'Not exposed' }, { name: 'Secrets', status: 'ok', detail: 'In env vars' }] }
  const columns = [
    { id: 'todo', title: 'To Do', emoji: '📋', color: '#f59e0b', tasks: [{ id: '1', title: 'Contact Kenny', priority: 'high' }, { id: '2', title: 'Calendar Integration' }] },
    { id: 'progress', title: 'In Progress', emoji: '🔄', color: '#667eea', tasks: [{ id: '3', title: 'Security Monitoring', priority: 'high' }] },

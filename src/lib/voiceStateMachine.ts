  ...
  switch (ctx.state) {
    ...
    case 'idle': {
      switch (event.type) {
        ...
        case 'TTS_STARTED':
          // Manual speak() when idle (e.g. PTT mode after mic off)
          return {
            context: {
              ...ctx,
              state: 'speaking',
    ...
    case 'listening': {
      switch (event.type) {
        case 'MIC_OFF': {
          ...
          // If there's accumulated text, send it before going idle (PTT release)
          const textOnOff = (ctx.pendingText || ctx.transcript || '').trim();
          if (textOnOff) {
            effects.push({ type: 'SEND_MESSAGE', text: textOnOff });
            return {
              context: {
                ...ctx,
                state: 'processing',
    ...
    case 'processing': {
      switch (event.type) {
        case 'MIC_ON':
          // User pressed mic/PTT while processing — force back to listening
          effects.push({ type: 'START_RECOGNITION' });
          effects.push({ type: 'LOG', message: 'MIC_ON during processing — restarting listen', level: 'info' });
          return {
            context: {
              ...ctx,
              state: 'listening',
        ...
        case 'RESPONSE_RECEIVED':
          // Always speak response (autoSpeak check is in the effect executor)
    ...
    case 'speaking': {
      switch (event.type) {
        case 'MIC_ON':
          // User pressed mic/PTT while speaking — barge-in, stop TTS, start listening
          effects.push({ type: 'STOP_TTS' });
          effects.push({ type: 'REVOKE_BLOB_URLS' });
          effects.push({ type: 'START_RECOGNITION' });
          effects.push({ type: 'LOG', message: 'MIC_ON during speaking — barge-in', level: 'info' });
          return {
            context: {
              ...ctx,
              state: 'listening',
